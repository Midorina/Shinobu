...
class Logging(commands.Cog):
    def __init__(self, bot: MidoBot):
        ...
        self.messages_to_cache = Queue()
        self.move_cache_to_db.start()

    @tasks.loop(minutes=1.0)
    async def move_cache_to_db(self):
        def gen(q: Queue()):
            for i in range(q.qsize()):
                yield self.messages_to_cache.get()

        await LoggedMessage.insert_bulk(self.bot, gen(self.messages_to_cache))
    ...
    def get_guild_id_out_of_event(self, arg):
        if isinstance(arg, (discord.Role, discord.Member)):
            ...
            channel: discord.TextChannel = self.bot.get_channel(arg.channel_id)
    ...
    async def base_logging_func(self, logging_type: LoggedEvents, *args):
        ...
        if guild_settings.logging_is_enabled:
            e = None
            content = None
            file = None
            ...
            if logging_type is LoggedEvents.MEMBER_JOIN:
                ...
                if not payload.cached_message:
                    msg = await LoggedMessage.get(self.bot, payload.message_id)
                else:
                ...
                if not payload.cached_messages:
                    msgs = await LoggedMessage.get_bulk(self.bot, payload.message_ids)
                else:
    ...
    async def on_message(self, msg: discord.Message):
        self.messages_to_cache.put(msg)
    ...
    async def log(self, ctx: mido_utils.Context):
